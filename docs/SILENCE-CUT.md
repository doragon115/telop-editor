...
[cut] 元の音声トラックを抜き出しています
[cut] 元音声: out/silence-cut/source_audio.m4a（19.8MB）
[cut] 解析用の音声を作っています
[cut] ここから先は元動画を読みません。消して構いません: ~/Movies/VID_....mp4
...
**元動画が要るのは最初の1分だけです。** `音声を抜き出しています` の行が終われば、
以降の段は元動画を一切読みません（抜き出した `source_audio.m4a` から切ります）。

2.6GB の動画で空きが足りない場合は、音声を抜いた時点で元動画を消せます。

```bash
npm run cut -- ~/Movies/元動画.mp4 audio    # 音声を抜くところまで
rm ~/Movies/元動画.mp4                       # ここで 2.6GB 戻る
npm run cut -- dummy --install               # 続きを実行（元動画は読まない）
```

抜き出した音声は再エンコードせずコピーするので、**音質は元のまま、17分で20MB前後**です。
...
| `source_audio.m4a` | 元の音声トラックのコピー。書き出しの元。**これがあれば元動画は不要** |
